...
def save_uploaded_file(file, save_name, size_limit):
    ...
    for chunk in file.file:
        real_file_size += len(chunk)    # Chunk size (default 1MB) set by starlette https://github.com/encode/starlette/blob/master/starlette/datastructures.py#L412
        if real_file_size > size_limit:
            logging.info("File has reached size {} > limit {}".format(real_file_size, size_limit))
            raise HTTPException(
                status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, detail="{} is too large".format(file.filename)
            )
        temp.write(chunk)
    temp.close()
...
@app.post("/simulate/")
async def simulate(n_lv: int = Form(5, title="Number of LV networks (up to 20)", ge=0, le=20),
