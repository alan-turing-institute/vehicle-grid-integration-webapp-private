...
from .funcsPython_turing import fillplot, get_path_files,\
                get_path_dirs, sff, listT, mtOdict, dds, csvIn, mtDict
...
pi2 = np.pi*2
ps = pi2/10
ic00_prof = 0.8 + 0.15*(np.cos(-xx*pi2 - pi2/2 + ps) \
...
class turingNet(snk.dNet):
    ...
    """
    def __init__(self,frId=40,frId0=None,rundict={}, mod_dir=os.path.join(os.getcwd(), "_network_mod")):
        ...
        snk.dNet.__init__(self,frId=frId,tests=[],yBdInit=False,
                        prmIn={'nomLdLm':1.0,'yset':None}, mod_dir=mod_dir)
    ...
    def set_base_tags(self,):
        ...
        self.pmryLnsi = [i for i,b in enumerate(objattrcache['LNS']['Bus1']) 
                                                            if b==pmryBus]
    ...
    def set_ldsi(self,dgd={},):
        ...
        for vv in lvmv:
            ...
            _ = [setattr(self.ldsi[k],'n'+vv,len(self.ldsi[k][vv]) ) 
                                            for k in list(self.ldsi.kw)]
    
    def plotXvNetwork(self,xv='mv',pType=None,pnkw={},net=None,):
        ...
        if pType in ['B','s','p','q',] or not txtOpts is None:
            lvntwkopts = {'color':lvn_clr,'marker':'.','ms':10,'mew':1,'ls':'',
                                                'zorder':30,'mfc':'None',}
        ...
        if txtOpts=='all':
            ...
            self.plotLoadNos(icMvBuses,ax=ax,aln=aln,fontsize=txtFs,)
            self.plotLoadNos(rsMvBuses,ax=ax,aln=aln,fontsize=txtFs,)
            if len(lvn_dict)==0:
                self.plotLoadNos(lvNtwkBuses,ax=ax,aln=aln,fontsize=txtFs,)
            else:
                self.plotLoadNos(lvNtwkBuses,ax=ax,aln=aln,fontsize=txtFs,
                                                                bbox=lvn_dict)
        ...
        if pType=='B':
            ...
            mnmx = [-1,1.2]
            c_lv, c_mv = [cm.viridis((i-mnmx[0])/np.diff(mnmx)[0]) 
                                                        for i in [0,1]]
            ...
            plt_lv, = plt.plot(np.nan,np.nan,'.',color=c_lv,mec='k',
                                                        mew=0.3,ms=10,)
            plt_mv, = plt.plot(np.nan,np.nan,'.',color=c_mv,mec='k',
                                                        mew=0.3,ms=10,)
            xLg, = plt.plot(np.nan,np.nan,**lvntwkopts)
            lbls = ['Res. demand,\nlumped','I&C demand,\nlumped',
                                                'Res. demand,\nLV Modelled',]
            plt.legend([plt_lv,plt_mv,(plt_lv,xLg)],lbls,)
        ...
        if pType in ['s','p','q',] and xv=='mv':
            ...
            mvPwrs_S = np.array(objattrcache['LDS']['kW']) \
                            + 1j*np.array(objattrcache['LDS']['kvar'])
            ...
            scrs = {k:v for k,v in zip(mvBuses+lvmvBuses,
                                            mvPwrsSel+lvmvPwrsSel)}
            cbt0s = {
                    'p':'Load, kW',
                    'q':'Reactive pwr, kVAr',
    ...
    def plotLvNtwx(self,ax,lvn_clr):
        ...
        for bus in self.ckts.ldNo:
            ax_scat = ax.scatter(*self.busCoords[bus],s=self.fPrm_['pms'],
                facecolors='None',marker='.',zorder=30,edgecolors=lvn_clr,
                                linewidth=1.2,)
        return ax_scat
        
    def ldsi2buses(self,ldsi_idx):
        """Convert ldsi_idx indexes to bus names for use with plotting funcs."""
        return [b[0].split('_')[-1] for b in 
                    vecSlc(d.getObjAttr(d.LDS,AEval='BusNames'),ldsi_idx)]
    ...
    def plotLoadNos(self,buses,ax=None,aln=None,**kwargs):
        ...
        for ii,bus in enumerate(buses):
            ax.text(*self.busCoords[bus],bus,
                                verticalalignment=aln[ii],**kwargs,)
        
        return ax
    
    def setupXvPlots(self,xv,net=None,):
        ...
        if xv=='mv':
            ...
            lraw = []
            for ff in feeders:
                dn = os.path.join(dn0,'lvns_coords',f'network_{lvn}',
                                                            f'Feeder_{ff}')
            ...
            if xv=='lv':
                ...
                for l in lraw:
                    ...
                    buses = [l[0].split(f'Bus{i}=')[1].split(' ')[0] 
                                                        for i in range(1,3)]
    ...
    def set_load_profiles(self,tsp,):
        ...
        ***NB***: this function is sometimes dependent on the the current state 
        ...
        self.dmnd = Bunch(
           {k:Bunch({vv:np.zeros((self.ldsi[k]['n'+vv],tsp['n']))
                                        for vv in ['lv','mv',]} ) 
        ...
        assert(len(rsev)==tsp['n']); assert(len(icev)==tsp['n'])
        
        # Residential demand rs
        if tsp['rs']['lv']=='crest' and self.ldsi.rs.nlv>0:
            # Loop through the crest data and assign profiles
            self.dmnd.rs.lv = np.array([r0[:,i % r0.shape[1]] 
                                        for i in range(self.ldsi.rs.nlv)])
        ...
        if tsp['ovnt']['lv']=='ee':
            self.dmnd.ovnt.lv = np.array([rsev 
                                        for i in range(self.ldsi.ovnt.nlv)])
        ...
        if tsp['dytm']['mv']=='ee':
            ...
            self.dmnd.dytm.mv = np.array([icev*lds0[i]*0.10/max(icev)
    ...
    def load_ee_data():
        ...
        rsev_,icev_ = [csvIn(os.path.join(fnev,f'{ss}.csv'),hh=False)
                                    for ss in ['rsev-week','icev-week']]
        rsev, icev = [np.array(dd).astype('float')[:144,1]/1e3 
                                                for dd in [rsev_,icev_]]
        return rsev, icev
    
    def record_solution(self,opts=None,):
        ...
        """
        sln = Bunch(mtDict(['Spri','Ssec',
                            'Ltot',
                            'Vmv','Vsb','Vld','Vic',
                            'Tap','Cnvg','Slds','VsrcMeas',
                            ]))
        ...
        sln.VlvLds = [np.array(vecSlc(vlds,v)) 
                                        for v in self.mvlvi.lv_n.values()]
    ...
    def get_lds_kva(self,tsp_n,dtypes=None,):
...
class modify_network():
    ...
    def modify_dss_files(self,nd,):
        ...
        lv_relpath = os.path.relpath(os.path.join(self.mod_dir, self.dnout, "lvNetworks"), self.destination)
        nlvTot = lv_txt.count('\n')
        ...
        lv_lst = [s.replace('redirect','!redirect') if i in II else s 
        ...
        lds_lst = [s.replace('False','True') if getLdi(s) in IIlds else s
...
def unzip_networks(dest_dir, n_id):
    ...
    blob_service_client = BlobServiceClient.from_connection_string(os.environ.get("NETWORKS_DATA_CONTAINER_READONLY_CONNECTION_STRING"))
    container = os.environ.get("NETWORKS_DATA_CONTAINER_READONLY")
