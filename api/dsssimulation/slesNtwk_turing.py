...
from .funcsMath_turing import aMulBsp, tp2ar, vecSlc, pf2kq, vmM, mvM, schurSolve, \
    calcDp1rSqrt, rerr, sparseInvUpdate, dp1rMv, calcVar, magicSlice,\
    sparseBlkDiag
from .funcsDss_turing import dssIfc, updateFlagState, pdIdxCmp, getVset, phs2seq
from .funcsPython_turing import fPrmsInit, whocalledme, saveFigFunc, sff, \
    struct, mtList, mtDict, cleanFolder, structDict, fn_ntwx, fn_root, \
                                        o2o, rngSeed, sff_legacy
import dss
...
class dNet:
    ...
    fdrsLoc = {5:join('13Bus_copy\\IEEE13Nodeckt_z'),
    ...
    fdrNos = [np.nan,4,5,6,6,8,2,7,2,6,6,5,3,4,6,7,4,7,9,5,5,5,6,5,2,3]
    ...
    fPrm_ = {
          ...
          'ttlOn':False,
          'cms':{'v':cm.RdBu,'p':cm.GnBu,'q':cm.PiYG,'n':cm.Blues,},
          'pms':50,
          }
    
    # 3 phase plotting scale of hexagons
    sfDict3ph = {'13bus':6,'123bus':60,'123busCvr':60,
                          'epriK1cvr':40,'34bus':80,'n1':4,'eulv':2,
                          'epri5':90,'epri7':50,'n10':3,'n4':3,'n27':4,
                          '33bus':12}
    

    
    def __init__(self, frId=30, yBdInit=True, prmIn={}, 
                 mod_dir=os.path.join(os.getcwd(), "_network_mod"), **kwargs):
        ...
        self.vKvbase = d.get_Yvbase(self.YZ,
                                    test=('ykvbase' in self.tests))[3:]
        ...
        self.getYset(yset=self.prm['yset'],state=self.state0,
                                    testYbus=('ybus' in self.tests))
        ...
        if yBdInit:
            ...
            self.Vnl = -spla.spsolve(self.Ybus[3:,3:],\
                                    self.Ybus[3:,:3]).dot(d.getVsrc())
    ...
    def updateLdsCount(self):
        ...
        sIdx, self.LDSidx, self.LDS_ = d.getPwrIdxs(d.LDS,self.YZv,)
    ...
    def updateGenCount(self):
        _ , self.GENidx, self.GEN_ = d.getPwrIdxs(d.GEN,self.YZv,)
        # self.pyIdx, self.pdIdx = sIdx
    ...
    def getYset(self,yset='both',**kwargs):
        ...
        if (yset=='both' and 'Ybus' in sKeys and 'Yprm' in sKeys) or \
                                                        yset in sKeys:
            capTap_ = getattr(self,'YbusCapTap').copy()
            self.updateYset(capTap_,capTap,yset,)
        else:
            self.calcYset(state,yset,)
    ...
    def updateYset(self,capTap_,capTap,yset='both',):
    ...
    def calcYset(self,state,yset='Ybus',):
        ...
        if yset in ['Yprm','both']:
            Ybus2,Yprm = d.buildYmat(d.PDE,state['capTap'],self.YZidxs,1,)[:2]
    ...
    def getIdxPhs(self):
        ...
        """
        self.iIdxPhsY = np.zeros(len(self.state0['LdsY'])
                                   + len(self.state0['GenY']),dtype=int)
        self.iIdxPhsD = np.zeros(len(self.state0['LdsD'])
                                   + len(self.state0['GenD']),dtype=int)
    ...
    def initialiseDssCkt(self):
        ...
        d.DSSText.Command='Batchedit load..* vminpu=0.02 vmaxpu=50'\
                             + ' model=4 status=variable cvrwatts='\
             +str(self.prm['pCvr']) + ' cvrvars='+str(self.prm['qCvr'])
    ...
    def buildSpHmat(self):
        ...
        self.Hmat = sparse.coo_matrix( (dataSet,(iSet,jSet)), 
                                            (self.nPd,self.nV) ).tocsr()
    ...
    def getCktFn(self,printF=False,frId=None):
        ...
        if len(str(frId))<3 or len(str(frId))>=4:
            if frId in [5,6,7,8,9,14,17,18,19,20,21,22,23,
            ...
            elif frId in list(range(60,67)) + list(range(80,87)) \
                                                    + list(range(90,97)):
            ...
            self.fdrsLoc[frId] = join(mvIdx,
                                        'master_1ph_'+mvIdx+'_slesNtwk.dss')
    ...
    def getYprmBd(self,**kwargs,):
        ...
        idxShuff = sparse.coo_matrix((
                np.ones(len(yzIdxs)),(np.arange(len(yzIdxs)),yzIdxs)),
                            shape=(len(yzIdxs),self.nV+3),dtype=int).tocsr()
    ...
    def getYlli(self,capTap,**kwargs):
        ...
        if capTap is None:
            ...
            Ylli_ = spla.inv(self.Ybus[3:,3:]).toarray()
            capTap_ = self.YbusState['capTap']
            dNet.saveCache(fn,{'Ylli':Ylli_,'capTap':capTap_},
                                                    cache=self.caches[C])
    ...
    def getCacheFn(self,C,getDn=False,prms={}):
        ...
        if not getDn:
            if C in ['Ybus','Ylli','cktInfo',]:
                fn = os.path.join(dn,self.caches[C]['fn_']+self.feeder+'.pkl')
            elif C in ['Vmlv']:
                lm = prms['LoadMult']
                fn = os.path.join(dn,self.caches[C]['fn_'] + \
                    self.feeder + '_' + f'{lm:.3f}'.replace('.','-') + '.pkl')
        else:
            fn = dn
        return fn
    ...
    def getUbMats(self,test=False):
        ...
        """
        (self.seqBus,self.seqBusPhs,self.vKvbaseSeq),self.Vc2Vcub = \
                                                d.getVc2Vcub(self.YZidxs)
        self.nVseq = len(self.seqBusPhs)
        self.nVps = len(self.seqBus)
        if test: self.testVc2Vcub()
    ...
    def xydt2x( self,YY,DD,TT,dim=1 ):
        ...
        if YY.ndim==1:
            ...
            if YY.shape[0]==self.nPy*2:
                XX = np.r_[ YY[:self.nPy], DD[:self.nPd],
                            YY[self.nPy:], DD[self.nPd:],
                            TT ]
        if YY.ndim==2:
            if dim==0:
                XX = np.r_[ YY[:self.nPy], DD[:self.nPd],
                ...
                XX = np.c_[ YY[:,:self.nPy], DD[:,:self.nPd],
                            YY[:,self.nPy:], DD[:,self.nPd:],
                            TT ]
        return XX
    
    def x2xydt( self,XX,xs=None,nPydt=None, ):
    ...
    def x2xc(self,X,conj=False):
        ...
        if conj:
            val = np.r_[ X[:self.nP] - 1j*X[self.nP:self.nS] , 
                                        X[self.nS:self.nS+self.nT] ]
        else:
            val = np.r_[ X[:self.nP] + 1j*X[self.nP:self.nS] , 
                                        X[self.nS:self.nS+self.nT] ]
        return val
    
    def state2xc(self,state,conj=False,):
    ...
    def state2x(self,state,):
        ...
        kGen = 1e3
        injY = np.r_[kLds*np.array(state['LdsY']),
                     kGen*np.array(state['GenY'])]
        injD = np.r_[kLds*np.array(state['LdsD']),
                     kGen*np.array(state['GenD'])]
    ...
    def updateRecord(self,uRcrdIdx=None):
        ...
        self.slnr.VsrcMeas = np.r_[self.slnr.VsrcMeas,
                                            np.array([self.sln.VsrcMeas])]
        
        if uRcrdIdx is None:
            self.slnr.Vc = np.r_[self.slnr.Vc,np.array([self.sln.Vc])]
        else:
            self.slnr.Vc = np.r_[self.slnr.Vc,
                                        np.array([self.sln.Vc[uRcrdIdx]])]
    ...
    def cMat2rMat( cMat,pmConj=False ):
        ...
        if type(cMat) is np.ndarray:
            if not pmConj:
                rMat = np.block([[cMat.real,-cMat.imag],
                                        [cMat.imag,cMat.real]])
            else:
                rMat = np.block([[cMat.real,cMat.imag],
                                        [cMat.imag,-cMat.real]])
        if type(cMat) in [ sparse.csr.csr_matrix,sparse.csc.csc_matrix ] :
            if not pmConj:
                rMat = sparse.bmat([[cMat.real,-cMat.imag],
                                        [cMat.imag,cMat.real]])
            else:
                rMat = sparse.bmat([[cMat.real,cMat.imag],
                                        [cMat.imag,-cMat.real]])
    ...
    def runMc(self,pdf,method,lms=[0.1,0.6],**kwargs):
        ...
        # Initialise 
        cRes = structDict({kw:structDict({lm:None for lm in lms_}) \
                                                    for kw in self.cns.kw})
        self.cResO = [(kw,lm,cn) for kw in self.cns.kw for lm in lms_ \
                                                    for cn in ['L','U']]
        self.getCresN()
        ...
        for i,pGen in enumerate(pdfMcAll):
            if method=='prgMc':
                ...
                cRes,S = self.linMcCalc(pGen,cRes,pml,)
            elif method=='dssMc':
                print(i)
                cRes,S = self.dssMcCalc(pGen,cRes,lms_,lms,state_,)
            elif method=='dssMc_em':
                cRes,S = self.dssMcCalc_em(cRes,V_[i],)
            
            feas[i], fFrac[i], = self.feasCalc(cRes,nMc,)
    ...
    def VmcFn(self,method,lms,pdf,):
        ...
        fn = os.path.join(sd,'_'.join([self.feeder,method,]+_lms+_N)+'.pkl')
        return fn
    ...
    def linMcCalc(self,pGen,cRes,pml,):
        ...
        for cns,(A,b) in self.pmlPairs.items():
            if not type(A) is list:
                ...
                if DA.dtype==float:
                    lNet.updateCresCns(cRes[cns],pml[b],Ax=DA,)
                ...
                lNet.updateCresCns(cRes[cns],Bs=pml[b_],Ax=DA,\
                                             Cx=DC,Ds=pml[d_],cplxFrac=True)
    ...
    def processLinMdl(self,loadMults,):
    ...
    def feasCalc(self,cRes,nMc,**kwargs):
        ...
        cResFs = {}
        for kw,lm,cn, in self.cResO:
            if cn=='U':
                if self.cResN[kw]==0:
                    maxVal = np.ones((nMc,))
                else:
                    maxVal = np.max(cRes[kw,lm],axis=1)
                cResFs[kw,lm,cn,] = maxVal>cns[kw,cn,]
            elif cn=='L':
                if self.cResN[kw]==0:
                    minVal = np.ones((nMc,))
                else:
                    minVal = np.min(cRes[kw,lm],axis=1)
                cResFs[kw,lm,cn,] = minVal<cns[kw,cn,]
        self.cResFs = cResFs
    ...
    def setupPlots(self):
        ...
        regSrc = ['epri24','8500node','123bus','123busCvr','epriJ1',
                                                        'epriK1','epriM1']
        self.srcReg = 1 if self.feeder in regSrc else 0
        
        # Update fPrm_['pms'] if wanted.
        plotMarkerDict = {'13bus':150,'34bus':150,'eulv':100,'epri5':10,
                            'epriK1cvr':15,'epriK1':15,'8500node':8,}
    ...
    def plotNetwork(self,pType=None,**kwargs):
        ...
        if pType in 'n':
            ...
            scores, minMax0 = self.getScrVals(score0,'s','sum')
            minMaxAbs = np.max(np.abs( 
                        np.array([np.nanmax(list(scores.values())),
            ...
            scoresFull = list(scores['1'].values()) \
                                    + list(scores['2'].values()) \
                                    + list(scores['3'].values())
            minMaxAbs = np.max(np.abs( 
                                np.array([np.nanmax(scoresFull),
                                          np.nanmin(scoresFull)]) ) )
            minMax0 = [-minMaxAbs,minMaxAbs] 
            cbTtl0 = '$Q$, kVAr' # Positive implies capacitive
    ...
    def plotNetColorbar(self,ax,minMax,cbTtl=None,nCbar=150):
        ...
        x0 = [xlim[0]-1]
        y0 = [ylim[0]-1]
        cntr = ax.contourf( np.array([x0*2,x0*2]),np.array([y0*2,y0*2]), 
                                        np.array([minMax,minMax[::-1]]),
                                            nCbar,cmap=self.fPrm_['cmap'] )
        ax.set_xlim(xlim)
        ax.set_ylim(ylim)
        # cbar = plt.colorbar(cntr,shrink=0.75,
                            # ticks=np.linspace(minMax[0],minMax[1],5))
        cbar = plt.colorbar(cntr,shrink=0.6,
                            ticks=np.linspace(minMax[0],minMax[1],5))
    ...
    def plotBuses(self,ax,scores,minMax,edgeOn=True):
        cmap = plt.cm.viridis if self.fPrm_['cmap'] is None \
                                                    else self.fPrm_['cmap']
        ...
        plt.scatter(x0scr,y0scr,marker='.',color=xyClr,
                            zorder=+10,s=self.fPrm_['pms'])
        if edgeOn: 
            plt.scatter(x0scr,y0scr,marker='.',
                                        zorder=+11,s=self.fPrm_['pms'],
                ...
                if not np.isnan(self.busCoords[bus][0]):
                    if np.isnan(scores[bus]):
                        ...
                        if minMax==None:
                            ...
                            score = (scores[bus]-minMax[0]) \
                                                    /(minMax[1]-minMax[0])
                        
                        xyClr = xyClr + [cmap(score)]
            
            self.hex3phPlot(ax,x0scr,y0scr,xyClr,phscr,
                                        sf=self.sfDict3ph[self.feeder])
            # plt.scatter(x0scr,y0scr,marker='.',Color=xyClr,
    ...
    def hex3phPlot(self,ax,x,y,xyClr,xyPhs,sf):
        ...
        # phsOffset =  {'1':np.exp(1j*np.pi/6),'2':np.exp(1j*5*np.pi/6),
                                                # '3':np.exp(1j*-np.pi/2)}
        phsOffset = {'1':np.exp(1j*0),'2':np.exp(1j*2*np.pi/3),
                                                '3':np.exp(1j*4*np.pi/3)}
        patches = []
        for i in range(len(x)):
            brSf = 1.15
            xyPts = np.c_[ sf*(xy0 + brSf*phsOffset[xyPhs[i]]).real \
                    + x[i],sf*(xy0 + brSf*phsOffset[xyPhs[i]]).imag + y[i]]
            #NB putting zorder here doesn't seem to work vvvv
            patches.append(Polygon(xyPts,True,fill=1,color=xyClr[i])) 
            patches.append(Polygon(xyPts,True,fill=0,linestyle='-',
                                            linewidth=0.4,edgecolor='k'))
            patches.append(Polygon([[x[i],y[i]],
                                     [x[i]+sf*brSf*phsOffset[xyPhs[i]].real,
                                      y[i]+sf*brSf*phsOffset[xyPhs[i]].imag]],
                                      False,color='k',linewidth=1.0 ))
        ...
        if self.nT>0:
            ...
            for regBus in self.regBuses:
                ...
                if not np.isnan(regCoord[0]):
                    ax.plot(regCoord[0],regCoord[1],'r',marker=(6,1,0),
                                                                zorder=+5)
                    # ax.annotate(str(i),(regCoord[0],regCoord[1]),zorder=+40)
                else:
                    print('Could not plot regulator bus'+regBus
                                                        +', no coordinate')
                i+=1
        else:
    ...
    def plotSub(self,ax,pltSrcReg=True):
        ...
        if np.isnan(srcCoord[0]):
            print('Nominal source bus coordinate not working,' 
                                                    +' trying z appended...')
            try:
        ...
        if not np.isnan(srcCoord[0]):
            ax.plot(srcCoord[0],srcCoord[1],'k',marker='H',
                                                markersize=8,zorder=+20)
            ax.plot(srcCoord[0],srcCoord[1],'w',marker='H',
                                                markersize=3,zorder=+21)
            if self.srcReg and pltSrcReg:
                ax.plot(srcCoord[0],srcCoord[1],'r',marker='H',
                                                markersize=3,zorder=+21)
            else:
                ax.plot(srcCoord[0],srcCoord[1],'w',marker='H',
                                                markersize=3,zorder=+21)
...
class lNet(dNet):
    ...
    *Note that this can be used for voltage flicker, setting x[iTaps] = 0
    ...
    def __init__(self,frId=5,tests=[],):
        """Basic initialisation using dNet class, not using any tests."""
        dNet.__init__(self,frId,tests=tests,)
    ...
    def lineariseSystem(self,mtype='fpl',tests=['bsc'],**kwargs ):
        ...
        if linSet in ['all','bare','bareMv','sqr',]:
            Mt0 = self.newTapModel(state)
        
        if linSet in ['all','bare',]:
            if mtype=='fpl':
        ...
        if linSet in ['all']:
            ...
            _,dYprm_dtSet = self.getdYdt(yset='Yprm',)
    ...
    def loadLinModel(self,loadInfo={}):
        ...
        filename = '_'.join([self.feeder,'{:.0%}'.format(
                        loadInfo['LoadMult']),loadInfo['mtype']])
    ...
    def saveLinModel(self,filename=None):
        if filename is None:
            filename = '_'.join([self.feeder,'{:.0%}'.format(
                            self.mdl.state['LoadMult']),self.mdl.mtype])
    ...
    def fplLinM(self,state):
        ...
        self.getYlli(capTap=state['capTap'],)
        
        My0_ = mvM( self.Ylli,1/Vh.conj() )[:,self.pyIdx]
        Md0_ = mvM( aMulBsp(self.Ylli,self.Hmat.T), 
                                        1/(self.Hmat.dot(Vh.conj())) )
    ...
    def createTapModel(self,state=None):
        ...
        dVdt_cplx = np.zeros((d.DSSCircuit.NumNodes - 3,
                                                d.RGC.Count),dtype=complex)
    ...
    def fotLinM(self,state,**kwargs):
        ...
        Vh = d.setNtwkState(state)[3:]
        fotMats = self.getFotMatrices(state,)
        ...
        if mvOnly:
            ...
            B1,B2 = fotMats[4:]
            YL0 = self.Ybus[3:,:3]
            B1_ = vmM(Vh,YL0.conj()).dot(
                            sparse.csc_matrix(lvFullData['dmvdx'].conj()))
            ...
            M5i = sparse.dia_matrix( (1/(self.Hmat.dot(Vh)),0),
                                    shape = (self.nPd,self.nPd) ).tocsr();
    ...
    def getFotMatrices(self,state,):
        ...
        M1 = sparse.dia_matrix( ( (self.Hmat.T).dot(iDconj),0 ),
                                            shape=(self.nV,self.nV) ).tocsr();
        M2 = vmM(Vh,self.Hmat.T)
        M3 = vmM( Vh, YLL.conj() )
        M4 = sparse.dia_matrix( ( (YL0.conj()).dot(d.getVsrc().conj()) 
                                    + (YLL.conj()).dot(Vh.conj()), 0),
                                            shape=(self.nV,self.nV) ).tocsr();
        
        M5 = sparse.dia_matrix( (self.Hmat.dot(Vh),0),
                                    shape = (self.nPd,self.nPd) ).tocsr();
        M5i = sparse.dia_matrix( (1/(self.Hmat.dot(Vh)),0),
                                    shape = (self.nPd,self.nPd) ).tocsr();
        ...
        Bpy = sparse.coo_matrix(
            (-np.ones(self.nPy),(self.syIdx[:self.nPy],np.arange(self.nPy))),
                                    shape=(self.nV*2,self.nPy)).tocsc()
    ...
    def schurShift(self,fotMats,mtype,):
        ...
        DD_ = sparse.bmat([[AA[idxJ][:,idxJ],BB[idxJ]],
                           [CC[:,idxJ],     DD        ],]).tocsc()
    ...
    def fastTest(self,):
        ...
        for mSel in self.mdl.lmSet:
            ...
            if c_vrbl[1] in ['Vmlv','|Vmlv|']:
                ...
                dMdlSa = [M_K1.dot(x1) for M_K1,x1 in 
                                        zip(getattr(self.mdl,mSel),xx1)]
                dMdlSb = [C_D.dot(F.dot(xx)) for C_D,F in 
                                    zip(getattr(self.mdl,c_vrbl[2]),
                                        getattr(self.mdl,c_vrbl[3]))]
            ...
            sln = mtDict(['act','actV','apx','err','errV',])
            sln['kk'] = kk
            sln = structDict(sln)
            for k in sln.kk:
                lm = k*self.mdl.state['LoadMult']
                state['LoadMult'] = lm
                state['GenY'] = list( lm*np.array( state0['GenY'] ) \
                                                    /state0['LoadMult'] )
                state['GenD'] = list( lm*np.array( state0['GenD'] ) \
                                                    /state0['LoadMult'] )
                d.setNtwkState(state)
    ...
    def cpfTest(self,**kwargs):
        ...
        if cpfType=='S':
            ...
            if c_vrbl[1] in ['Vmlv','|Vmlv|']:
                ...
                dMdlSa = [M_K1.dot(x1) for M_K1,x1 in 
                                        zip(getattr(self.mdl,lm),xx1)]
                dMdlSb = [C_D.dot(F.dot(xM)) for C_D,F in 
                                    zip(getattr(self.mdl,c_vrbl[2]),
                                        getattr(self.mdl,c_vrbl[3]))]
        ...
        if pShw:
            fig,axs = plt.subplots(5,figsize=(4,7),sharex=True,
    ...
    def newTapModel(self,state):
        ...
        f11 = sparse.dia_matrix((( (self.Hmat.T).dot(iDconj) - xx),[0]),
                                                    (self.nV,self.nV))
        ...
        dYbus_dtSet, _ = self.getdYdt(yset='Ybus',)
    ...
    def getdYdt(self,yset='both',test=False,):
        ...
        while i:
            ...
            if yset in ['both','Ybus']:
                ...
                col = []
                data = []
                d.updateRowColData(row,col,data,rgcPrm['YZa'],
                                            rgcPrm['YZb'],rgcPrm['dYdtV'])
                dYbus_dt = sparse.coo_matrix((data,(row,col)),
                                        shape=(nYbus, nYbus),dtype=complex)
            ...
            if yset in ['both','Yprm']:
                ...
                iIdxs = [self.YprmFlws.index(yz) \
                                for yz in self.getFlwNms(rgcPrm,'YprmI')]
            ...
            if test:
                ...
                for prmType,yptype,mat in zip(['YprmV','YprmI'],
                                              ['dYdtV','dYdtI'],
                                              ['dYbus_dt','dYprm_dt'],
                                               ):
                    Yprm_DT = []
    ...
    @staticmethod
    def calc_dYdt(rgcPrm,t,prmType='YprmV',):
        ...
        return np.block([[0*y11,-y12*((1+t)**-2)],
    ...
    def ldcUpdate(self,mdl,):
        ...
        zVlts = d.getRgcZvlts()
        rgcFlws, rgcFlwsIdx = d.getRgcFlwIdx(self.YprmFlws,self.YZvIdxs,)
...
class mlvNet(lNet):
    ...
    def __init__(self,frId=39,reloadDss=True,):
        ...
        if reloadDss:
            # get ckt info
            prmIn = {'yset':'Ybus'}
            dNet.__init__(self,frId,yBdInit=False,prmIn=prmIn,tests=[],)
            self.getCktInfo()
    ...
    def lineariseSystem(self,state=None,tests=[],saveLin=True,rvl=None):
        ...
        lNet.lineariseSystem(self,mtype='fot',tests=tests,
                                            linSet='bareMv',state=state)
        ...
        for i in range(self.ckts['N']):
            lvNet, mdls = self.linLvCkt(i,'lvRankFczn',tests=tests,
                                        LoadMult=state['LoadMult'],rvl=rvl)
            ...
            yyhat = self.mdl.M1[-1].dot(xx1[i]) + \
                   self.mdl.C[-1].dot(self.mdl.F[-1].dot(xx)) + self.mdl.i[-1]
            argI = o2o(yyhat.conj()/np.abs(yyhat))
            
            self.mdl.K1.append((mdls[0]*argI).real)
            self.mdl.D.append((mdls[1]*argI).real)
            self.mdl.j.append(np.abs(yyhat) - self.mdl.K1[-1].dot(xx1[i])
        ...
        if saveLin:
            C = 'Vmlv'
            fn = dNet.getCacheFn(self,C,
                                prms={'LoadMult':self.mdl.state['LoadMult']})
            data = {
                    'mdl':self.mdl,
                    'ckts':self.ckts,
                    'mlvIdx':self.mlvIdx,
    ...
    def loadMlvMdl(self,lm=None,_pf=False):
        ...
        if _pf:
            ...
            N_s = self.mlvNsplt
            self.mdl._K1 = [K1[:,:n_s] + pf*K1[:,n_s:]
                                        for K1,n_s in zip(self.mdl.K1,N_s)]
            self.mdl._F = [F[:,:self.nP] + pf*F[:,self.nP:]
    ...
    def loadMlvFlatMdl(self,lm,test=False,testKw={'pShw':False},):
        ...
        mdlPrms = {k:deepcopy(getattr(self.mdl,k)) \
                            for k in ['K1','FF','D','F',]}
        ...
        self.mdl.j = [vv1[i] - self.mdl.K1[i].dot(xx1[i]) \
    ...
    def getCtype(fn,lvFrid,ldNo):
        ...
        dn = join(dn0,'lvNetworks','_'.join(['network',
                                    lvFrid,'1',ldNo]))
    ...
    def getLvCktInfo(self,state=None,fn0=None,):
        ...
        """
        cktKeys = ['trnIdx','lvFrid','ldNo','YZvIdx','YZmvIdx','lvPrp',
                    'V','Vbase','pwrs','LvYZsIdx','LvYZs','LvYZvIdx',
                                'LvYZv','LvYZvIdx_V','LvYZsIdx_S','ctype',]
        cc = mtDict(cktKeys)
        cc['N'] = 0
        ...
        while i:
            if not d.TRN.Name.split('_')[0]=='transformer':
                ...
                cc['trnIdx'].append(i)
                cc['lvFrid'].append(int(d.TRN.Name.split('_')[0][2:-1]),)
                cc['ldNo'].append(ldNo)
                cc['ctype'].append(mlvNet.getCtype(fn0,
                                            cc['lvFrid'][-1],cc['ldNo'][-1]))
                cc['N']+=1
                ...
                cc['LvYZsIdx'].append(np.array([i for i,yz in 
                            enumerate(self.YZs) if yz[:nPrp]==lvPrp]))
                cc['LvYZs'].append([yz[nPrp:] for yz in self.YZs 
                                                        if yz[:nPrp]==lvPrp])
                
                cc['LvYZvIdx'].append(np.array([i for i,yz in 
                                   enumerate(self.YZv) if yz[:nPrp]==lvPrp]))
                cc['LvYZv'].append([yz[nPrp:] for yz in self.YZ 
                                                        if yz[:nPrp]==lvPrp])
                
                cc['LvYZvIdx_V'].append(np.array([i 
                                for i in cc['LvYZvIdx'][-1] 
                                                    if i in self.sIdx]))
                cc['LvYZsIdx_S'].append(np.r_[cc['LvYZsIdx'][-1],
                                              cc['LvYZsIdx'][-1]+self.nP])
    ...
    def linLvCkt(self,i_,mtype='lvFull',**kwargs):
        ...
        ***SIDE EFFECT***
        ...
        if not rvl is None:
            updateFlagState(state0,flagVal=1.0,rvl=rvl,
                                        lm=state0['LoadMult'],IP=True)
        ...
        if mtype=='bare':
            ...
            if mtype=='lvFull':
                MyLv,MdLv,aLv = lvNet.fotLinM(state0,lvFull=True,
                                                    lvFullData=lvFullData,)
                Mlv = self.xydt2x(MyLv,MdLv,Mt)
                mdl = [Mlv,aLv]
            if mtype=='lvFullSqr':
                MyLv,MdLv,aLv = lvNet.fotLinM(state0,lvFullSqr=True,
                                                    lvFullData=lvFullData,)
            ...
            if mtype=='lvRankFczn':
                ...
                My1,Md1,_ = lvNet.fotLinM(state0,sqr=True,)
                M1 = self.xydt2x(My1,Md1,Mt[lvNet.sIdx])
                
                # Then get the tank factorization stuff
                CC,_,Vh = lvNet.fotLinM(state0,lvRankFczn=True,)
                # FF = lvFullData['dmvdx'].conj()
                FF = np.r_[lvFullData['dmvdx'].real,
        ...
        if len(tests)>0:
            ...
            if 'fczn' in tests and mtype=='lvRankFczn':
                _,mdl_, = self.linLvCkt(i_,'lvFullSqr',tests=[], 
                                        LoadMult=state0['LoadMult'],rvl=rvl)
    ...
    def getVmlvLin(self,state=None,xx=None,mSel='K1',):
        ...
        if mSel[0]=='_':
            if xx.shape[0]==self.nS:
                print('Running K1')
                return self.getVmlvLin(state=None,xx=xx,mSel='K1',)
    ...
    def mlvPlot(self,Vset,ax=None,pKwArgs={},ff=None,ylm=None):
        ...
        plt.vlines(np.r_[-0.5,np.cumsum(self.mlvNsplt)+0.5],*ylm,
    ...
    def getRndX(self,pp=3,frac=0.1,nSc=1,reseed=False,method='nom',):
        ...
        if method=='nom' or method[1:] in ['__','pf']:
            ...
            int1 = hcDraws.frac2int(f1,self.nPlv,)
            int3 = hcDraws.frac2int(f3,self.nPlv,)
            int6 = self.nPlv - int3 - int1

            # Draw the location 'types'
            prms = hcDraws.genPermutation(self.nPlv,nSc,)
            set1 = prms<int1
            set3 = (prms>=int1)*(prms<(int3+int1))
            set6 = prms>=(int1+int3)

            # Draw the demand locations
            pSet = [-1e3*pp*self.rngs.binomial(1,frac,size=(ii*nSc)) 
                                for pp,ii in zip([1,3,6],[int1,int3,int6])]
            q6 = pSet[-1]*pf2kq(pf6)

            # Set the demands
            xxPlv = np.zeros((self.nPlv,nSc))
            for setsel,psel in zip([set1,set3,set6],pSet,):
                xxPlv[setsel]  = psel
            ...
            prms = hcDraws.genPermutation(self.ckts.N,nSc,)
            intN = hcDraws.frac2int(fckt,self.ckts.N,)
            fcktTru = intN/self.ckts.N
            
            setN = prms<intN
            
            idxPset = [np.concatenate(vecSlc(self.ckts.LvYZsIdx,cktIdx))
                                                for cktIdx in setN.T]
    ...
    def getNset(self,Vpu,DVlin,Vp,Vm,DVp,nSc,):
    ...
    def processAllResults(fn,Vp,Vm,DVp):
        ...
        vltnsAll = {key:[np.any(np.c_[vmax>Vp,vmin<Vm,dvmax>DVp],axis=1)
        ...
        for fridA,fridB in zip(rr.fridSetA,rr.fridSetB):
            ...
            vltns[fridA] = [sum(np.any(np.c_[vvA,vvB],axis=1))/rr.nSc 
...
class hcDraws:
    ...
    def __init__(self,feeder,dMu=None,**kwargs):
        ...
        meta.update({'name':pdfName,'prms':prms,'mu_k':mu_k,'nMc':nMc,
    ...
    def halfLoadMean(scale,xhyN,xhdN):
        ...
        Mu0_y = -scale*roundI*np.round(xhyN[:xhyN.shape[0]//2]/roundI  \
                                    - 1e6*np.finfo(np.float64).eps)
        Mu0_d = -scale*roundI*np.round(xhdN[:xhdN.shape[0]//2]/roundI \
                                    - 1e6*np.finfo(np.float64).eps)
    ...
    def genPdfMcSet(self,nMc,prmI,getMcU=False,**kwargs):
        ...
        if self.meta.name=='gammaWght':
            ...
            pdfMc0 = np.random.gamma(shape=k,
            ...
            if not hasattr(self,'_nMc') or self._nMc!=nMc:
                ...
                self.genGamma = np.random.gamma(shape=slrPrm['k'],
                                    scale=slrPrm['th_kW'],size=(nGen,nMc))
                self._nMc = nMc
            ...
            pGen = np.random.gamma(shape=slrPrm['k'],
                                scale=slrPrm['th_kW'],size=(nGen,nMc))
    ...
    def genPermutation(nGen,nMc,seed=0):
        ...
        for i in range(nMc):
            idxs = rng.permutation(range(nGen),)
            permutation[:,i] = idxs
...
def pltCns(self,pdf,lms,dataSets,**kwargs):
    ...
    fig,axs = plt.subplots(nrows=self.cns.N,ncols=2*len(lms),
                            sharex=True,sharey=True,figsize=(11,7))
    axs = axs.flatten()
...
def pltCnsPair(self,pdf,lms,dataSets,**kwargs):
    ...
    for kw,ax,i in zip(self.cResO,axs,range(len(self.cResO))):
        ...
        for data,lbl,clrs,m in zip(dataSets,dataLbls,clrPrs,mults):
            ...
            ax.fill_betweenx(100*pdf.meta.prms,m*data.fsFr[:,i],
                                color=clrs[1],alpha=0.6,label=lbl)
            ax.fill_betweenx(100*pdf.meta.prms,m*data.fsFr[:,i],
                    np.ones(pdf.meta.nP[0])*m*100,color=clrs[0],alpha=0.2)
...
def testLin(fdrT,cs):
    ...
    self = lNet(fdrT,)
    d = self._d
    pdf = hcDraws(self.feeder,nMc=cs['nMc'],
                                        prms=cs['prm'])
    nSc = pdf.meta.nP[0]
    Mu = pdf.getMuCov(prmI=np.arange(nSc),nGen=self.nP)[0]
    self.allGenSetup()
...
def test_getPmlNsets(self,pdf,lms):
    ...
    pml = self.processLinMdl(lms)
    Nstds = self.getCnsNstds(pdf,pml,)
    ...
    Yvar = self.calcAdotDp1rVar(calcVar(A),A.dot(np.ones(A.shape[1])),
                                                            XcovSqrt)
    print(PgenCov)
    print(XcovSqrt)
    Ky = dp1rMv(A,XcovSqrt)
