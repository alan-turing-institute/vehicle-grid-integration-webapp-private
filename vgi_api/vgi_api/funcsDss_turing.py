...
class dssIfc():
    ...
    def getYmat(d,capTap=None,vbs=False,):
    ...
    def buildYmat(d,obj,capTap=None,YZidxs=None,vbs=False,):
        ...
        Ybus = sparse.coo_matrix((data, (row, col)), shape=(nYbus, nYbus),
                                                                dtype=complex)
        Ybus = Ybus.tocsc()
    ...
    def ybusPrimer(d,):
        ...
        return ldsEnbld, genEnbld,
    
    def ybusReenable(d,ldsEnbld,genEnbld,):
    ...
    def get_YprmFlws(d):
        ...
        while i:
            ...
            YpFlws.extend([d.AE.Name+'__'+busSel for busSel in 
                                                    nodeSetAI+nodeSetBI])
            i = d.PDE.Next
    ...
    def createYbus(d,capTap=None,vbs=True):
        ...
        Ybus0 = sparse.coo_matrix((SysYV,(SysYK,np.zeros(len(SysY_dct),
                                                            dtype=int))))
    ...
    def getVc2Vcub(d,YZidxs):
        ...
        idxsX = np.kron(np.ones(3,dtype=int),
                                    np.arange(len(seqBusPhs),dtype=int))
        ...
        Vc2Vcub = sparse.coo_matrix((outs,(idxsX,idxsY)),
                            shape=(len(seqBusPhs),nYZ)).tocsr()
        return (seqBus,seqBusPhs,seqBuskV),Vc2Vcub
    
    def getPwrIdxs(d,obj,YZv,):
    ...
    def get_Yvbase(d,YZ,test=False):
        ...
        rto = np.abs((V.reshape((-1,1,)) - Vbase))
    ...
    def getYdObj(d,obj,yStr,dStr):
        ...
        recordSln = d.SLN.Converged and \
                                (not d.SLN.SystemYChanged)
        if recordSln:
            ydLds[yStr+'dss'] = []
            ydLds[dStr+'dss'] = []
    ...
    def getRgcFlwIdx(d,YprmFlws,YZvIdxs,):
    ...
    def addGenerators(d,genBuses,delta):
        ...
        for genBus in genBuses:
            ...
            if not delta: # ie wye
                ...
                d.DSSText.Command=''.join(['new generator.',genName,
                    ' phases=1 bus1=',genBus,' kV=',genKV,' kW=0.5 ',
                ...
                d.DSSText.Command=''.join(['new generator.',genName,
                    ' phases=1 bus1=',genBuses,' kV=',genKV,' kW=0.6',
    ...
    def buildIncidenceMatrix(d,Yprm):
        ...
        iMatrix = sparse.coo_matrix(
                                (np.ones(len(idxsJ),dtype=bool),(idxsI,idxsJ)),
                                    shape=(len(brchNs),len(busNs)),dtype=bool)
...
def updateFlagState(state,flagVal=1.0,rvl=1.0+0.0j,lm=1.0,IP=False,w0=True,):
...
def xfmrYprm(cA,cB,t,kV,kVA,ppm_a,ll,xhl):
    ...
    if cA==1:
        ...
        if cA=='D' and cB=='Y':
            yprm = np.block([[Y2_(yt),-Y3_(yt).T/t,],
                              [-Y3_(yt)/t,Y1_(yt)/(t**2)]])
            aF1 = 1/3
            aF2 = 0.5
        if cA=='Y' and cB=='Y':
            yprm = np.block([[Y1_(yt),-Y1_(yt).T/t,],
                              [-Y1_(yt)/t,Y1_(yt)/(t**2)]])
            aF1 = 0.5
            aF2 = 0.5
...
def plotZbusSns(self,ZbusPuKva,zbusIs=None):
    if zbusIs is None: zbusIs = [0,3,6,9]
    ...
    for zbusI,ax in zip(zbusIs,axs.flatten()):
        ...
        ax.text(1-0.03,-0.35,'$10^{'+str(minThng)+'}$'); ax.plot(1,0,'k|',markersize=7)
        ax.text(np.sqrt(3)-0.03,-0.35,'$10^{'+str(minThng+2)+'}$'); ax.plot(np.sqrt(3),0,'k|',markersize=7)
        for idx,r in enumerate(circR):
            ax.plot(np.sqrt(r)*np.cos(tSp),np.sqrt(r)*np.sin(tSp),linewidth=0.25,color=cm.matlab( idx % 6))
        ...
        ax.plot( rds*np.cos(ngl), rds*np.sin(ngl),'ko',markersize=2.5,markeredgewidth=0.6,markerfacecolor='None' )
        ax.plot( rds[zbusI]*np.cos(ngl[zbusI]), rds[zbusI]*np.sin(ngl[zbusI]),'bx',markersize=3 )
        ax.set_aspect('equal'); 
...
setFdrs = { # (ntwk, [fdrs to take out])
           ...
           15:[1,2,4,], # too big - remove 139 loads
...
def commentBadFdrs(ntwk,masterText,oneFlag=False):
    ...
    for keyset in keySet:
        if ntwk in keyset.keys():
            ...
            newlines = []
            for line in lines:
                condition = [f'Feeder_{ff}' in line or f'feeder{ff}' in line
                                                            for ff in fdrs]
