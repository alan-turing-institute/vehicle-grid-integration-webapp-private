...
def vmM(vec,Mat): 
    ...
    if type(Mat) is np.ndarray:
        ...
        MatOut = Mat*vec[:,None] 
    elif type(Mat) in [sparse.csc.csc_matrix, sparse.csr.csr_matrix] and \
                                                type(vec) is np.ndarray:
...
def sparse2lowRank(A):
    ...
    Nr = len(idxs)
    U_T = np.zeros((A.shape[0],Nr,),dtype=complex)
    V = np.zeros((Nr,A.shape[0],),dtype=complex)
