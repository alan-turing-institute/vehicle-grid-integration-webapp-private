...
# avoid writing out the preamble twice...
...
class turingNet(snk.dNet):
    ...
    def __init__(
        ...
        fn0 = None if frId0 is None else self.getCktFn(frId=frId0)[1]
        self.frId0 = frId0
        snk.mlvNet.getLvCktInfo(
            self,
            fn0=fn0,
        )
...
class modify_network:
    ...
    def modify_dss_files(
        ...
        for fn in fn_copy.values():
            fn_src = os.path.join(self.dn_dst, fn)
            fn_dst = fn_src + self.dnout
            shutil.copy(fn_src + ".dss", fn_dst + ".dss")

        # Get resulting file names
        fn_lds, fn_mstr, fn_lv, fn_rgc, fn_xfmr = [
            os.path.join(self.dn_dst, fn_copy[vv] + self.dnout + ".dss")
            for vv in [
                "lds",
                "mstr",
                "lv",
        ...
        lv_txt = lv_txt.replace(
            "lvNetworks", os.path.join("..", self.fldr_ntwk, "lvNetworks")
        )
        lv_lst = lv_txt.split("\n")
...
def unzip_networks():
    ...
    fn_root
    ntwks = get_path_files(
        os.path.join(fn_root, "network-zips"),
        mode="paths",
    )
    for ntwk in ntwks:
        ntwk_dn = os.path.join(snk.fn_ntwx, os.path.split(ntwk)[1])
        if not os.path.exists(ntwk_dn.split(".")[0]):
            # with zipfile.ZipFile(ntwk_dn, 'r') as zip_ref:
            with zipfile.ZipFile(ntwk, "r") as zip_ref:
                zip_ref.extractall(snk.fn_ntwx)

                # Check directory separator style and edit from \ -> / if necessary
                dss_utils.changeDirectorySeparatorStyle(
                    ntwk_dn.split(".")[0], verbose=True
                )
